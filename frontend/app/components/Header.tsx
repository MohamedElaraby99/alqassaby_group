  ...
  return (
    <header 
      className={`fixed w-full top-0 z-50 transition-all duration-300 ${
        isScrolled || isAboutPage  ? 'shadow-lg' : 'bg-black/30 backdrop-blur-md'
      }`}
      style={isScrolled || isAboutPage ? { backgroundColor: '#a01623' } : {}}
    >
      {/* Main Navigation */}
        ...
        <div className="flex justify-between items-center py-3 sm:py-4">
          ...
          <Link href={`/${locale}`} className={`flex items-center flex-shrink-0 ${locale === 'en' ? 'gap-2' : 'gap-0.5 sm:gap-1 mr-[-115px]'}`}>
            ...
            <div className="text-sm sm:text-lg md:text-2xl font-bold">
              <span className="transition-colors text-yellow-300 whitespace-nowrap drop-shadow-md" style={{ textShadow: '0 2px 4px rgba(0, 0, 0, 0.4)' }}>
