  ...
  return (
    <header 
      className={`fixed w-full top-0 z-50 transition-all duration-300 shadow-lg ${
        isScrolled ? '' : 'bg-black/20 backdrop-blur-md lg:bg-transparent lg:backdrop-blur-none'
      }`}
      style={isScrolled ? { backgroundColor: '#bf1e2e' } : {}}
    >
      {/* Main Navigation */}
          ...
          <ul className="hidden lg:flex items-center gap-6 xl:gap-8">
            {menuItems.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className="font-semibold transition-colors text-white hover:text-yellow-300"
                >
            ...
            <ul className="space-y-2">
              {menuItems.map((item, index) => (
                ...
                >
                  <Link
                    href={item.href}
                    className="block px-4 py-3 rounded-lg font-semibold text-gray-800 hover:bg-primary hover:text-white transition-all duration-200 transform hover:scale-105"
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
                    {locale === 'ar' ? item.label : item.labelEn}
                  </Link>
